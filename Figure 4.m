...
R0 = compute_R0_from_params(k);
fprintf('  R0      = %.6f\n', R0);

tforward = 0:dt:Time_Epidemic; 

[HIV_Cases, HIV_Diagnose, AIDS_Cases, AIDS_deaths, I, D] = FiniteDifference_HIV_Between_Host_Model(k, dt, Time_Epidemic);
...
% Goal triangle markers (draw once)
...
% Arrow labels (kept as-is)
ax = gca; ax_pos = get(ax, 'Position'); xlim_ax = xlim(ax); ylim_ax = ylim(ax);
x2fig = @(x) ax_pos(1) + (x - xlim_ax(1)) / diff(xlim_ax) * ax_pos(3);
y2fig = @(y) ax_pos(2) + (y - ylim_ax(1)) / diff(ylim_ax) * ax_pos(4);
arrow_offset = 40;
annotation('textarrow', [x2fig(2025 - 2.5), x2fig(2025)], ...
    [y2fig(goal2025 + arrow_offset), y2fig(goal2025)], ...
    'String', '2025 Year Target: 9.3k', 'FontSize', 12, 'Color', [0 0.5 0]);
annotation('textarrow', [x2fig(2030 - 3), x2fig(2030)], ...
    [y2fig(goal2030 + arrow_offset), y2fig(goal2030)], ...
    'String', '2030 Year Target: 3k', 'FontSize', 12, 'Color', [0 0 0.6]);
...
annotation('textarrow', [x2fig(2025 - 2.5), x2fig(2025)], ...
    ...
    'String', '2025 Year Target: 9.6k', 'FontSize', 12, 'Color', [0 0.5 0]);
annotation('textarrow', [x2fig(2030 - 3), x2fig(2030)], ...
...
for k = 1:N-1
    ...
    I(k+1) = (I(k)+dt*(beta*S(k+1)*I(k)/Ntot(k)+S(k+1)*Bd_Int(k))) ...
             /(1+(mu+alpha+rho)*dt);
...
function R0 = compute_R0_from_params(params)
    % Unpack parameters
    beta         = params(2);
    gamma        = params(3);
    rho          = params(4);
    ...
    % Constants
    mu     = 0.02;     % natural mortality
    kappa  = 52;       % scaling factor
    d_tau  = 0.52;     % tau step
    Treatmeant_Age = 20*52;

    % Within-host initial conditions (same as main model)
    T0  = 167; 
    Ti0 = 85; 
    V0  = 69643;

    % Within-host simulation for V(tau)
    [tau, y] = ode23s(@(t,y) HIV_Within_Host(y), 0:d_tau:Treatmeant_Age, [T0; Ti0; V0]);
    V_tau       = y(:,3);
    beta_d_tau  = beta_d_coeff .* V_tau;

    % gamma_d = 0 everywhere
    gamma_d_tau = zeros(size(tau));

    % Inner integral: ∫_0^tau (mu + gamma_d(s)) ds
    inner_int = cumtrapz(tau, mu + gamma_d_tau);

    % Exponential term
    exp_term = exp(-(1/kappa) .* inner_int);

    % Outer integral
    integral_term = trapz(tau, beta_d_tau .* exp_term);

    % Final R0 formula
    R0 = beta/(mu + gamma + rho) + (rho/(kappa*(mu + gamma + rho))) * integral_term;
end
